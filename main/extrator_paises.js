...
if (!NOME_EVENTO) {
  ...
  console.error("Exemplo: node extrator_paises.js event_14000");
  process.exit(1);
}

const NOME_ARQUIVO_SAIDA = `historico_paises_${NOME_EVENTO}.csv`;
...
function criarPayloadPaises(data, nomeEvento) {
  return {
    "entity": { "propertyId": "151460007", "identityBlendingStrategy": 2 },
    "requests": [
      {
        "dimensions": [{ "name": "country", "isSecondary": false }, { "name": "event_name", "isSecondary": true }],
        ...
        "cardId": "35q584nmIt", // Mantive o ID do card original, geralmente não afeta se a estrutura for aceita
        "requestGrandTotal": true,
        "dateRanges": [{ "startDate": data, "endDate": data }], // Data dinâmica
        "rowAxis": { "fieldNames": ["country"], "sorts": [{ "fieldName": "event_count", "sortType": 1, "isDesc": true, "pivotSortInfos": [] }, { "fieldName": "country", "sortType": 1, "isDesc": false, "pivotSortInfos": [] }, { "fieldName": "total_users", "sortType": 1, "isDesc": false, "pivotSortInfos": [] }], "limit": 200, "offset": 0, "metaAggTypes": [] },
  ...
  try {
    ...
    if (linhas) {
      linhas.forEach(linha => {
        // O valor [ { "value": "" } ] ou [ { "value": "Brazil" } ]
        let pais = linha.dimensionCompoundValues[0].value;
        if (pais === "") {
          pais = "(not set)"; // Define um valor padrão para País vazio
        }
        
        const contagemEventos = linha.metricCompoundValues[0].value;
        const totalUsuarios = linha.metricCompoundValues[1].value;
  
        linhasCSV.push(`"${data}","${pais}",${contagemEventos},${totalUsuarios}`);
      });
    }
...
async function buscarHistorico(nomeEvento) {
  console.log(`Iniciando extração de Países para o evento: ${nomeEvento}...`);
  const stream = fs.createWriteStream(NOME_ARQUIVO_SAIDA);
  stream.write("Data,Pais,ContagemDeEventos,TotalDeUsuarios\n");
  ...
  while (dataAtual <= dataFim) {
    ...
    console.log(`Buscando Países para ${dataFormatada}...`);
    
    const payload = criarPayloadPaises(dataFormatada, nomeEvento);
    ...
    try {
      ...
      if (linhasCSV.length > 0) {
        ...
        console.log(`  > Nenhum dado encontrado para ${dataFormatada}.`);
      }
    } catch (err) {
  ...
  stream.end();
  console.log(`Extração de Países concluída! Arquivo salvo em: ${NOME_ARQUIVO_SAIDA}`);
}
